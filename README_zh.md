...
| Nexus 原生 runtime | `client.NewOptions().WithRuntime(client.RuntimeNXS)` |
...
默认命令发现仍兼容 Claude Code，会查找 `claude`。需要切到 Go 原生 Nexus runtime 时，直接选择基于公开 release manifest 的 runtime resolver：
