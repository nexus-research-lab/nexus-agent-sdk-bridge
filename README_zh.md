...
| Nexus 原生 runtime | `client.NewOptions().WithCLIPath("nxs")` |
...
默认命令发现仍兼容 Claude Code，会查找 `claude`。如果要切到 Go 原生 Nexus runtime，先构建或安装 `nxs`，再通过 `WithCLIPath` 指定；宿主进程 `PATH` 受限时建议传绝对路径。
